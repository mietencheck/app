...
import { loadMockBeratungRecord } from "./mock-data";
import {
  BeratungRecord,
  MerkmaleByGruppe,
  MerkmalState,
...
export function BeratungPage() {
  ...
  useEffect(() => {
    ...
    loadMockBeratungRecord().then((record) => {
      if (!isMounted) return;
      setBeratungRecord(record);
      setState(structuredClone(record.merkmale));
    });
    ...
    <>
      <header className="border-b border-gray-6 shadow">
        <div className="container flex items-center justify-between py-3">
          <a href="">mietencheck.de</a>
          <Button size="sm">Sign out</Button>
        </div>
      </header>
      <main className="container py-4 flex flex-col gap-12">
